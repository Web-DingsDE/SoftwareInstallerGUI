...
$CheckBoxTeamViewerQS.Checked     = $false
...
$CheckBoxRenameComputer.Checked     = $true
...
If (Test-Path -Path $destinationBRZclient){ 
    ...
    $destinationBRZclientExists = "exist"
}
ELSE{ 
    ...
    $destinationBRZclientExists = "notexist"
}
}
...
function btnStart { 
    ...
    #### Testing Admin And Path
    if ((TestPath) -eq $false) {
        $LogWindow.text += "$(timestamp)   :FEHLER: Keine Admin-Rechte. Bitte Script als Administrator starten. `r`n"
        return
    }
    Else{
        $LogWindow.text += "$(timestamp)   :    OK    : Admin-Rechte vorhanden. `r`n"
    }
